...
Typespec *array_type;
    if(match_token(TOKEN_LBRACKET))
    {
       ...
       if (!is_token(TOKEN_RBRACKET)) {
        ...
        array_type = parse_type_base();
        array_type = new_typespec_array(pos, array_type, size);
        return array_type;
    }
...
StmtList parse_stmt_block(void) {
 ...
 bool is_ret_statement = false;
 char * temp_stream;
 Token temp_token;
bool is_end_of_func()
{

    if(is_keyword(return_keyword))
   {
    is_ret_statement = true;
    return true;
   }

temp_stream = malloc(sizeof(char ) * 1000);
    strcpy(temp_stream, stream);
   ...
   expect_token(TOKEN_NAME);
    ...
    if(is_keyword(fn_keyword))
    {
       

       stream = temp_stream;
       token = temp_token;
        is_ret_statement = false;
...
StmtList parse_stmt_block_no_curly_brace(void) {
    SrcPos pos = token.pos;
   // expect_token(TOKEN_LBRACE);
   // printf("%s\n", token.name);
    Stmt **stmts = NULL;
    while (!is_token_eof() && !is_end_of_func())//&& !is_token(TOKEN_RBRACE))
    {
...
AggregateItem parse_decl_aggregate_item(void) {
    ...
    if (match_keyword(struct_keyword)) {
        return (AggregateItem){
            ...
            //.internal_struct = parse_decl_struct(NULL, pos, DECL_STRUCT)
        };
       ...
       SrcPos tpos = token.pos;
        ...
        Typespec *type = p_get_type(name, tpos);
...
Decl *parse_decl_opt(void) {
    ...
    if (match_keyword(enum_keyword)) {
       return parse_decl_enum(NULL, pos);
       ...
       // if (match_token(TOKEN_RET)) {
        //if(match_keyword(i64_keyword)){

        //if(is_token(TOKEN_NAME)){
            
          //  if(strcmp(token.name, "i64") == 0){
        ...
        if (match_token(TOKEN_SEMICOLON)) {
            ...
            block = parse_stmt_block_no_curly_brace();
            is_incomplete = false;
        }
