...
Decl *new_decl_pp(SrcPos pos, Expr *expr, PP_StmtList block)
{

    //Decl *d = new(DECL_PP, pos, na);

}

Aggregate *
new_aggregate(SrcPos pos, AggregateKind kind, AggregateItem *items, size_t num_items) {
...
Stmt *new_stmt_null_return(SrcPos pos, Expr *expr) {
    Stmt *s = new_stmt(STMT_NULL_RETURN, pos);
