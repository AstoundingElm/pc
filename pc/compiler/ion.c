...
void init_package_search_paths(void) {
    const char *pchome_var = getenv("PCHOME");
    if (!pchome_var) {
    ...
    const char *pcpath_var = getenv("PCPATH");
    if (pcpath_var) {
        const char *start = pcpath_var;
        for (const char *ptr = pcpath_var; *ptr; ptr++) {
            if (*ptr == ';') {
...
void parse_env_vars(void) {
    const char *pcos_var = getenv("PCOS");
    if (pcos_var) {
        int os = get_os(pcos_var);
        if (os == -1) {
    ...
    const char *pcarch_var = getenv("PCARCH");
    if (pcarch_var) {
        int arch = get_arch(pcarch_var);
        if (arch == -1) {
