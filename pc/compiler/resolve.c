...
Operand operand_type(Type *type){
    return(Operand){
        .type = type,
        .is_const = true,
    };
...
Operand resolve_name_operand(SrcPos pos, const char *name) {
    ...
    if (sym->kind == SYM_VAR) {
        ...
        if(strcmp(sym->name, "uint") == 0){
           Type *uint_type = type_uint;
     return operand_lvalue(uint_type);//operand_const(uint_type, sym->val);
    }
        *( int * )0 = 0;
...
void add_internal_structs(Aggregate *aggregate)
{
                for(size_t i = 0; i < aggregate->num_items; i++){
...
bool is_package_dir(const char *search_path, const char *package_path) {
    ...
    for (dir_list(&iter, path); iter.valid; dir_list_next(&iter)) {
        ...
        if ((ext != iter.name && strcmp(ext, "p") == 0)) {
            dir_list_free(&iter);
            return true;
        }
    }
