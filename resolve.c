...
void add_package_decls(Package *package) {
    for (size_t i = 0; i < package->num_decls; i++) {
        ...
        if(decl->kind == DECL_STRUCT)
        {
            ...
            if(decl->aggregate->items[i].internal_struct  )
            {
                
               sym_global_decl(decl->aggregate->items[i].internal_struct);

               
            } /*else if(decl->aggregate->items[i].kind == AGGREGATE_ITEM_SUBAGGREGATE)
            {
               // printf("union\n");
               //exit(-1);
                Aggregate * agg = decl->aggregate->items[i].subaggregate;
                for(size_t a = 0; a < agg->num_items; a++)
                {

                
                sym_global_decl( agg->items[a].internal_struct);
                }
            }*/
